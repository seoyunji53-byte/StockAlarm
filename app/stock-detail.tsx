      ...
      {loading ? (
        ...
        <ScrollView style={styles.scroll}>
          <View style={styles.detailHeader}>
            ...
            {stockData && (
              <>
                <Text style={styles.bigPrice}>{stockData.price?.toLocaleString()}원</Text>
          ...
          <View style={[styles.card, { borderColor: '#B5D4F4', borderWidth: 1.5 }]}>
            ...
            {aiAnalysis ? (
              ...
              <TouchableOpacity style={styles.aiStartBtn} onPress={fetchAICoach} disabled={loadingAI}>
                {loadingAI ? (
...
function SimpleLineChart({ data, avgPrice, isUp, width: w }: {
  ...
  const h = 100;
  const prices = data.map((d: any) => d.close).filter(Boolean);
  ...
  return (
    <View style={{ alignItems: 'center' }}>
      <svg width={w} height={h + 20} viewBox={`0 0 ${w} ${h + 20}`}>
        <polygon
          points={`0,${firstY} ${points} ${w},${lastY} ${w},${h} 0,${h}`}
          fill={isUp ? '#E6F1FB' : '#fdf0ee'}
          opacity="0.6"
        />
...
const styles = StyleSheet.create({
  ...
  toggle: {
    width: 36, height: 21, borderRadius: 99, backgroundColor: '#D3D1C7'
  },
  toggleOn: { backgroundColor: '#185FA5' },
  dartItem: { paddingVertical: 8, borderBottomWidth: 0.5, borderBottomColor: '#f0ede8' },
  dartTitle: { fontSize: 12, fontWeight: '600', color: '#1a1814', lineHeight: 17 },
  dartDate: { fontSize: 10, color: '#9e9992', marginTop: 3 },
  newsItem: { paddingVertical: 8, borderBottomWidth: 0.5, borderBottomColor: '#f0ede8' },
